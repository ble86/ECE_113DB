...
static void get_square_magnitude(float32_t complex_vals[], float mag_vals[]) {
	for(int i = 0; i < FFT_SIZE; i++) {
		float real = complex_vals[2 * i];
		float imag = complex_vals[2 * i + 1];
		//float mag = sqrt((real * real) + (imag * imag));
		float mag = pow(real, 2.0) + pow(imag, 2.0); //(real * real) + (imag * imag);
		mag_vals[i] = mag;
	}
}
...
static void get_psd(float32_t complex_vals[], float periodogram[]) {
	...
	get_square_magnitude(complex_vals, mag_array);
	...
	*/
	...
	for (int i = 0; i < FFT_SIZE / 2; i++) {
		periodogram[i] = (mag_array[i] / periodogram_energy);
	}
